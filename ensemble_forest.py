# -*- coding: utf-8 -*-
"""Ensemble Forest.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1-CLg3U8cAjn88R8qYFdHsZm6AeOotK0n
"""
...
class BaRDT:
    ...
    def __init__(self, n_trees=40, min_samples_split=2,
                 max_depth=100, n_feats=None, weight=1, pen = 0):
    ...
    def fit(self, X, y):
        ...
        for i in range(self.n_trees):
            ...
            tr_svr = tree()
            tr_svr.fit_sv(X_samp, y_samp, self.pen, weight=self.weight, maximal_leaves=2*np.sqrt(X_samp.shape[0]*2/3))
...
class BoRDT:
    ...
    def __init__(self, no_of_trees=10, min_samples_split=2,
                 max_depth=100, n_feats=None, weight=1, pen = 0):
    ...
    def fit_predict(self,X_train, y_train, X_test):
        ...
        weight = [1/n] * n
        amount_of_say = [0] * self.no_of_trees
        y_test_pred = np.zeros((X_test.shape[0], self.no_of_trees))

        for i in range(self.no_of_trees):
            tr_svr = tree()
            tr_svr.fit_sv(X_train, y_train, self.pen, weight=self.weight, maximal_leaves=2*np.sqrt(n*2/3))
            y_train_pred = tr_svr.predict(X_train)

            no_of_total_error = np.sum(y_train != y_train_pred)
            if no_of_total_error == 0:
               no_of_total_error = 1
            err = no_of_total_error * weight[0]
            amount_of_say[i] = math.log((1-err)/err) * 0.5

            ind = np.full(n, -1)
            ind[y_train != y_train_pred] = 1

            y_test_pred[:,i] = tr_svr.predict(X_test)

            weight_upd = weight_update(amount_of_say[i],weight,ind,X_train,y_train)
            X_train = weight_upd['x_train']
            y_train = weight_upd['y_train']
        true_pred = [0] * np.shape(X_test)[0]
        for j in range(np.shape(X_test)[0]):
            amount_of_say = np.array(amount_of_say)
            tr = np.flatnonzero(y_test_pred[j,]==0)
            t = sum(amount_of_say[tr])
            fl = np.flatnonzero(y_test_pred[j,])
            f = sum(amount_of_say[fl])
            if t>=f:
               true_pred[j] = 0
            else:
               true_pred[j] = 1
